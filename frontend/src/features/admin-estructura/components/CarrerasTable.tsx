/**
 * CarrerasTable — ABM table for carreras with client-side filter.
 * Follows AsignacionesTable pattern. < 200 LOC.
 */
import { useState, useMemo } from 'react'
import type { CarreraRead, CarreraClientFiltros } from '../types'
import { EmptyState, StatusBadge } from '@/shared/components/ui'

interface Props {
  carreras: CarreraRead[]
  onEdit: (carrera: CarreraRead) => void
  onDelete: (id: string) => void
  isDeleting: boolean
}

const EMPTY_FILTROS: CarreraClientFiltros = {}

function applyFiltros(list: CarreraRead[], f: CarreraClientFiltros): CarreraRead[] {
...
export default function CarrerasTable({ carreras, onEdit, onDelete, isDeleting }: Props) {
  const [filtros, setFiltros] = useState<CarreraClientFiltros>(EMPTY_FILTROS)
  const filtered = useMemo(() => applyFiltros(carreras, filtros), [carreras, filtros])
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select
          ...
          onChange={(e) => setFiltros((f) => ({ ...f, estado: e.target.value as CarreraClientFiltros['estado'] }))}
          className={inputClass}
        >
      ...
      {filtered.length === 0 ? (
        <div data-testid="carreras-empty">
          <EmptyState title="No hay carreras para los filtros aplicados." />
        </div>
      ) : (
