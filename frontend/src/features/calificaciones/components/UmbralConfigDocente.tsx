/**
 * UmbralConfigDocente — configure the docente's override approval threshold.
 * Used by PROFESOR/COORDINADOR (scope propio). Sets asignacion_id (override) in the backend.
 * Shows a banner when inheriting from the default (is_default=true).
 * < 200 LOC.
 */
...
export default function UmbralConfigDocente({ materia_id }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {data?.is_default && (
        <div
          className="bg-yellow-50 border border-yellow-200 rounded px-3 py-2 text-sm text-yellow-800"
          data-testid="umbral-heredado-banner"
        >
          Heredando umbral por defecto de la materia: <strong>{data.umbral_pct}%</strong>.
          Podés configurar tu propio umbral a continuación.
        </div>
      )}
