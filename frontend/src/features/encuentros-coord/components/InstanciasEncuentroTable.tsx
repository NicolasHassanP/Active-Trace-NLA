 ...
 * Each row has Edit (opens EditarInstanciaDialog) and Delete (with confirmation)
 * action buttons wired to PATCH and DELETE endpoints.
...
import EditarInstanciaDialog from './EditarInstanciaDialog'
import { useBorrarInstancia } from '../hooks/encuentrosCoordHooks'
...
export default function InstanciasEncuentroTable({ instancias }: Props) {
  const [editingInstancia, setEditingInstancia] = useState<InstanciaEncuentroRead | null>(null)
  const borrarMutation = useBorrarInstancia()

  async function handleBorrar(inst: InstanciaEncuentroRead) {
    const confirmed = window.confirm(
      `¿Borrar la instancia "${inst.titulo}" del ${inst.fecha}? Esta acción no se puede deshacer.`,
    )
    if (!confirmed) return

    try {
      await borrarMutation.mutateAsync(inst.id)
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => void handleBorrar(inst)}
                      aria-label={`Borrar ${inst.titulo}`}
                    >
                      Borrar
                    </Button>
                  </div>
