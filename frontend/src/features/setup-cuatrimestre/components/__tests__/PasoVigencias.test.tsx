...
describe('PasoVigencias — render', () => {
  it('renders the form with data-testid paso-vigencias', () => {
    render(<PasoVigencias onSuccess={vi.fn()} onError={vi.fn()} />, { wrapper: makeWrapper() })
    expect(screen.getByTestId('paso-vigencias')).toBeTruthy()
  })
  ...
  it('renders materia_id as <select> with options by nombre', async () => {
    ...
    await waitFor(() => {
      const select = screen.getByTestId('vigencias-materia-id')
  ...
  it('renders carrera_id as <select> with options by nombre', async () => {
    ...
    await waitFor(() => {
      const select = screen.getByTestId('vigencias-carrera-id')
      expect(select.tagName.toLowerCase()).toBe('select')
  ...
  it('renders cohorte_id as <select> with options by nombre', async () => {
    ...
    await waitFor(() => {
      const select = screen.getByTestId('vigencias-cohorte-id')
      expect(select.tagName.toLowerCase()).toBe('select')
  ...
  it('materia options have correct values (ids) and labels (nombres)', async () => {
    ...
    await waitFor(() => {
      const select = screen.getByTestId('vigencias-materia-id') as HTMLSelectElement
