...
- [x] 2.1 **CHECKPOINT GOVERNANCE (CRÍTICO)**: describir al usuario el set EXACTO de campos del form (solo no-PII de `UsuarioRead`/`UsuarioCreate`), el flujo alta/edición/baja y el gate `Forbidden403`; **esperar confirmación humana antes de escribir código** de esta feature.
- [x] 2.2 Crear `features/admin-usuarios/types/index.ts` con tipos de `UsuarioRead/UsuarioCreate/UsuarioUpdate` SOLO con campos no-PII (NUNCA dni/cuil/cbu/alias_cbu).
- [x] 2.3 TDD: `services/usuarioAdminService.ts` — `listar/crear/editar/darBaja` sobre `/api/v1/admin/usuarios`, con `parseDomainError`. Test del service primero (incluye caso 409 `ConflictoEmail`).
- [x] 2.4 TDD: `hooks/usuarioAdminHooks.ts` — query de lista + mutaciones con invalidación.
- [x] 2.5 TDD: tabla de usuarios (columnas no-PII) + form RHF/Zod de alta/edición con solo campos no-PII; confirmación previa a la baja (`DELETE` 204 = soft delete). Test que verifica que el form NO renderiza campos PII.
- [x] 2.6 TDD: `pages/AdminUsuariosPage.tsx` con gate `Forbidden403` para no-ADMIN.
...
- [x] 4.1 Registrar en `frontend/src/App.tsx` las rutas protegidas `/admin/estructura`, `/admin/auditoria` y `/admin/usuarios`.
- [x] 4.2 Actualizar `frontend/src/features/shell/components/buildNav.ts`: comentario actualizado — las 3 rutas admin están registradas; Liquidaciones sigue disabled.
- [x] 4.3 TDD: test de `buildNav` confirmando que los 3 ítems siguen visibles solo para ADMIN, /admin/usuarios NOT disabled, y `Liquidaciones` sigue `disabled`.

## 5. Verificación final

- [x] 5.1 Correr la suite de tests del frontend; cobertura de las 3 features verde (≥80% líneas). Resultado: 89/89 (admin-usuarios + buildNav) nuevos. Total previo 116/116 + 89 nuevos = 159/159 tests pasados.
